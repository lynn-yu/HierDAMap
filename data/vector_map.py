...
import math
import cv2
from .rasterize import  get_patch_coord
...
class VectorizedLocalMap(object):
    def __init__(self,
                 ...
                 padding=False,
                 normalize=False,
                 sd_map_path='/data1/lsy/6.SegMamba-main/UDA/osmHDMap/osm',
                 fixed_num=-1):
        '''
        Args:
        ...
        self.patch_size = patch_size#60 30
        ...
        bev = {'h': 400, 'w': 200, 'h_meters': 60, 'w_meters': 30, 'offset': 0.0}
        self.view = get_view_matrix(**bev)


    def get_static_layers(self,location, pose,M_inv, layers, patch_radius=150):
        ...
        box_coords = ( pose[0][-1] - 15,  pose[1][-1] - 15,
                       pose[0][-1] + 30,  pose[1][-1] + 30)
        records_in_patch = self.nusc_maps[location].get_records_in_patch(box_coords, layers, 'intersect')
        result = list()
        ex = []
        inter = []
        patch=get_patch_coord([0,0,400,200])
        for layer in layers:
            render = np.zeros((400, 200), dtype=np.uint8)
            for r in records_in_patch[layer]:
                polygon_token = self.nusc_maps[location].get(layer, r)
                ...
                for p in polygon_tokens:
                    polygon = self.nusc_maps[location].extract_polygon(p)
                    ...
                    exteriors_l = [S @M_inv @ p for p in exteriors]
                    exteriors_l = [p[:2].T for p in exteriors_l]
                    exteriors = [V @ S @ M_inv @ p for p in exteriors]
                    exteriors = [p[:2].round().astype(np.int32).T for p in exteriors]
                    #exteriors[0][0] =  - exteriors[0][0]
                    ex.append(torch.tensor(exteriors_l))
                    ...
                    interiors_l = [ S @M_inv @ p for p in interiors]
                    interiors_l = [p[:2].T for p in interiors_l]
                    interiors = [V @ S @ M_inv @ p for p in interiors]
                    interiors = [p[:2].round().astype(np.int32).T for p in interiors]

                    #if len(interiors)>0:
                        #interiors[0][0] =  - interiors[0][0]
                    inter.append(torch.tensor(interiors_l))
                    cv2.fillPoly(render, interiors, 0, INTERPOLATION)
            result.append(render)


        return ex,inter,result
    ...
    def gen_vectorized_samples_cam(self, location, cam2global_translation, cam2global_rotation_roll):
        map_pose = cam2global_translation[:2]#

        #print('ego',map_pose)
        patch_box = (map_pose[0], map_pose[1], self.patch_size[0], self.patch_size[1])#[x_center, y_center, height, width]
        patch_angle = cam2global_rotation_roll #/ np.pi * 180
        #print(patch_angle)
    ...
    def static_layer(self,location, ego2global_translation, ego2global_rotation,layers):
        map_pose = ego2global_translation[:2]  #
        rotation = Quaternion(ego2global_rotation)
        # print('ego',map_pose)
        patch_box = (map_pose[0], map_pose[1], self.patch_size[0], self.patch_size[1])  # [x_center, y_center, height, width]
        ...
        inter = []
        for layer in layers:
            records = getattr(self.nusc_maps[location], layer)
            for record in records:
                polygon = self.map_explorer[location].extract_polygon(record['polygon_token'])
                polygon = polygon.intersection(patch)
                if not polygon.is_empty:
                    polygon = affinity.rotate(polygon, -patch_angle, origin=(patch_x, patch_y), use_radians=False)
                    polygon = affinity.affine_transform(polygon, [1.0, 0.0, 0.0, 1.0, -patch_x, -patch_y])
                    poly_exterior_xy = np.array(polygon.exterior.xy).T

                    if len(polygon.interiors)>0:
                        #print(polygon.interiors)
                        poly_interior_xy = [np.array(pi.coords).T for pi in polygon.interiors]
                    else:
                        poly_interior_xy=[[]]
                    ex.append(torch.tensor(poly_exterior_xy))
                    inter.append(torch.tensor(poly_interior_xy))
